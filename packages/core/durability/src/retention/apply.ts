...
export async function applyRetentionOverride(
  ...
  await executor.query(
    `UPDATE ${damatRelation("_damat_event_outbox")} SET "retention_ms"=$2,
  ...
  await executor.query(
    `UPDATE ${damatRelation("_damat_event_deliveries")} d SET "retention_at"=o."retention_at",
       "updated_at"=NOW() FROM ${damatRelation("_damat_event_outbox")} o
...
async function applyPipelineRetention(
  ...
  const retention = input.retentionMs === "forever" ? null : input.retentionMs;
  await executor.query(
    `UPDATE ${damatRelation("_damat_pipeline_runs")} r SET "retention_ms"=$2,"retention_at"=
       CASE WHEN $2::bigint IS NULL OR r."completed_at" IS NULL THEN NULL
         ELSE r."completed_at"+($2*INTERVAL '1 ms') END,"updated_at"=NOW()
     FROM ${damatRelation("_damat_pipeline_definitions")} d
