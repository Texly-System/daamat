...
- **Writes are validated against the model's columns.** Every generated write (`create`, `createMany`, `update`, `updateOne`, `upsert`, and `upsertMany`) validates before it reaches the repository. Auto-generated columns (primary key, autoincrement), columns with a default, and nullable columns are optional; nullable columns also accept `null`. Updates validate in `partial` mode, so only supplied fields are checked. Native `vector`/`halfvec` columns additionally require a finite numeric array with exactly the declared `dimensions`; failures identify the column and the array/type/finite or dimension problem. Ordinary PostgreSQL arrays retain their existing validation path.
