...
- Use the Repository pattern (`findMany`, `findOne`, `findById`, `create`, `update`, `delete`, `upsert`, `count`, `exists`, `findNearest`).
- Use native pgvector columns with `columns.vector(dimensions)` / `columns.halfVector(dimensions)`; modeled values are validated, serialized with the official adapter, and read back as `number[]`.
...
| `PgRepository`, `PgRepositoryConfig`                                                | class / type | Repository over a model: `findMany/findOne/findById/findManyByIds/create/createMany/update/updateOne/delete/deleteById/upsert/upsertMany/count/exists/findNearest`.                                                                                  |
