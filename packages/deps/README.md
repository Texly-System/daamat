...
`@damatjs/deps` is a thin façade over the third-party libraries the framework relies on — Hono, Zod, Effect, `pg`, pgvector, `ioredis`, `nanoid`, and `uuid`. Every other Damat package imports these through `@damatjs/deps/<lib>` instead of depending on the library directly. This gives the whole monorepo a single pinned version of each dependency, a single place to upgrade, and curated re-exports (for example, the `hono` subpath bundles the framework plus its commonly used sub-modules and `@hono/node-server`'s `serve`).
...
- You are writing a Damat package and need Hono, Zod, Effect, `pg`, pgvector, `ioredis`, `nanoid`, or `uuid`. Import from `@damatjs/deps/<lib>` so you inherit the pinned version.
...
| Export                  | Kind                 | Summary                                                                                                                                                                                                                                                                                               |
| ----------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@damatjs/deps`         | namespace re-exports | Root entry. Re-exports every library under a namespace: `hono`, `zod`, `effect`, `pg`, `pgvector`, `ioredis`, `nanoid`, `uuid` (e.g. `import { zod } from "@damatjs/deps"` then `zod.z`). Avoids name collisions; prefer the subpaths below.                                                                      |
...
| `@damatjs/deps/pgvector` | curated adapter      | Official pgvector-node `0.3.0` node-postgres adapter (`registerTypes`, `toSql`).                                                                                                                                                                                                                         |
